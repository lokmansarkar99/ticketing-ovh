...
import POSCounterTicketPrint from "@/pages/dashboard/printLabel/POSCounterTicketPrint";

interface IQuickTicketSearchProps {
  searchTicketInfo: any;
  setSearchTicketInfo: any;
}

const QuickTicketSearch: FC<IQuickTicketSearchProps> = ({
  searchTicketInfo,
}) => {
  ...
  useEffect(() => {
    if (searchTicketInfo) {
      const pendingTicket = localStorage.getItem("pendingQuickSearch");
      if (pendingTicket) {
        setTicketNumber(pendingTicket);
        setSearchTicketNumber(pendingTicket);
        localStorage.removeItem("pendingQuickSearch");
      }
    }
  }, [searchTicketInfo]);
...
export default QuickTicketSearch;
