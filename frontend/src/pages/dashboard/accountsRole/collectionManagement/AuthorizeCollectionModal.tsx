...
import { useAuthorizeReportFromAccountMutation } from "@/store/api/accounts/accountsDashboardApi";
...
const AuthorizeCollectionModal: FC<AuthorizeCollectionModalProps> = ({
  ...
  const onSubmit = async () => {
    ...
    const result = await authorizeReportFromAccount({
      reportId,
      body: {
  ...
  return (
    <FormWrapper
      heading={translate("অনুমোদন সংগ্রহ", "Authorize Report")}
      subHeading={translate(
        "সংগ্রহ অনুমোদনের জন্য নিচের তথ্য পূরণ করুন।",
        "Fill out the details below to authorize the report."
      )}
    >
      <h2 className="text-green-400 font-semibold">Cash on Hand: {amount}</h2>
      <form
        onSubmit={(e) => {
          e.preventDefault();
          onSubmit();
