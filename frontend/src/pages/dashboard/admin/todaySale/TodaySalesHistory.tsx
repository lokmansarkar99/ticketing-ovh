...
import PdfTodaySalesReport from "./pdf/PdfTodaySalesReport";
import TodaySalesExcel from "./excel/ExcelTodaySales";

interface salesDataProps {
  salesData: any;
}

const TodaySalesHistory: FC<salesDataProps> = ({ salesData }) => {
  const totalItems = salesData?.todaySalesHistory?.length || 0;
  ...
  const columns: ColumnDef<any>[] = [
    ...
    // },
    {
      ...
      size:80,
      cell: ({ row }) => {
        const value = row.original.order?.smsSend;
        return value ? "Yes" : "No";
      },
    },
  ];
    ...
    <div>
      ...
      >
                ...
                >
                  <DropdownMenuItem>
                    <PDFDownloadLink
                      document={
                        <PdfTodaySalesReport
                          result={salesData?.todaySalesHistory || []}
                          singleCms={singleCms}
                        />
                      }
        ...
        <DataTable
          ...
          pagination
          columns={columns}
          data={salesData?.todaySalesHistory || []}
        />
      </TableWrapper>
    </div>
  );
...
export default TodaySalesHistory;
