...
import PdfOnlineHistory from "./pdf/PdfOnlineHistory";
import { useGetSingleCMSQuery } from "@/store/api/cms/contentManagementApi";
import { Loader } from "@/components/common/Loader";
import TodayOnlineHistoryExcel from "./excel/ExcelTodayOnlineHistory";

interface salesDataProps {
  salesData: any;
}

const OnlineHistory: FC<salesDataProps> = ({ salesData }) => {
  const totalItems = salesData?.onlineHistory?.length || 0;
                ...
                >
                  <DropdownMenuItem>
                    <PDFDownloadLink
                      document={
                        <PdfOnlineHistory
                          result={salesData?.onlineHistory || []}
                          singleCms={singleCms}
                        />
                      }
                      fileName="today_online_sales_report.pdf"
                    >
                      {
                        //@ts-ignore
                        (params) => {
