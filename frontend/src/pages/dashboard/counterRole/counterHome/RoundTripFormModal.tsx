...
const RoundTripFormModal: FC<ICounterBookingFormProps> = ({
  ...
  const debouncedCoupon = useDebounce(currentCouponArg, 500);
  ...
  const calculateTotalDiscount = () => {
    ...
    bookingFormState?.selectedSeats?.forEach((seat: any) => {
    ...
    return {
      ...
      totalOriginalPrice: bookingFormState?.selectedSeats?.reduce(
        (sum: any, seat: any) => sum + seat.currentAmount,
        0
      ),
    };
  };
  ...
  const minimumPartialPayment = useMemo(() => {
    if (partialInfoData?.data?.partialPercentage) {
      return (
        (totalDiscountedPrice * partialInfoData.data.partialPercentage) / 100
      );
    }
    return 0;
  }, [totalDiscountedPrice, partialInfoData]);
  useEffect(() => {
    ...
    <section className=" w-full">
      <PageTransition>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {bookingFormState.selectedSeats?.length > 0 ? (
            <>
              <div className="my-1 px-4 py-2 bg-gray-50 rounded-lg border border-gray-200">
