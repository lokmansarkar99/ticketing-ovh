...
const PdfCoachWiseSalesReportSummery = ({ result, singleCms, userName, startDate, endDate }: any) => {
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.section}>
                    ...
                    <View style={styles.titleWraper}>
                        <Text style={styles.title}>Coach Wise Summery Report </Text>
                    ...
                    <View style={styles.table}>
                        <View style={styles.tableRow}>
                            ...
                            <Text style={[styles.tableHeader, { flex: 1 }]}>Total Seat Qty</Text>
                            <Text style={[styles.tableHeader, { flex: 1 }]}>Sold Seat Qty</Text>
                            <Text style={[styles.tableHeader, { flex: 1 }]}>Remaining Unsold</Text>
                        </View>
                        ...
                        {result?.map((sale: any, index: number) => (
                            <View style={styles.tableRow} key={sale.id} wrap={false}>
                                ...
                                <View style={{ flex: 1, ...styles.tableCol }}><Text>{sale?.seatQuantity}</Text></View>
                                <View style={{ flex: 1, ...styles.tableCol }}><Text>{sale?.soldSeatQty}</Text></View>
                                <View style={{ flex: 1, ...styles.tableCol }}><Text>{sale?.seatRemainingUnsold}</Text></View>
                            </View>
                        ))}
                        ...
                        <View style={[styles.tableRow, { borderTop: "1pt solid black" }]} wrap={false}>
                            ...
                            <View style={{ flex: 1, ...styles.tableCol, fontWeight: "bold" }}><Text>{result?.reduce((sum: number, s: any) => sum + (s.seatQuantity || 0), 0)}</Text></View>
                            <View style={{ flex: 1, ...styles.tableCol, fontWeight: "bold" }}><Text>{result?.reduce((sum: number, s: any) => sum + (s.soldSeatQty || 0), 0)}</Text></View>
                            <View style={{ flex: 1, ...styles.tableCol, fontWeight: "bold" }}><Text>{result?.reduce((sum: number, s: any) => sum + (s.seatRemainingUnsold || 0), 0)}</Text></View>
                        </View>
                    </View>
                </View>
...
export default PdfCoachWiseSalesReportSummery;