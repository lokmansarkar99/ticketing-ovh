...
const PdfCounterSalesReport = ({ result, singleCms, userName, startDate, endDate }: any) => {
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.section}>
                    ...
                    <View style={styles.titleWraper}>
                        <Text style={styles.title}>Counter Wise Sales Report </Text>
                    ...
                    <View style={styles.table}>
                        <View style={styles.tableRow}>
                            ...
                            <Text style={[styles.tableHeader, { flex: 1 }]}>Commission Fare</Text>
                        </View>
                        ...
                        {result?.map((sale: any, index: number) => (
                            <View style={styles.tableRow} key={sale.id} wrap={false}>
                                ...
                                <View style={{ flex: 1, ...styles.tableCol }}><Text>{sale?.date}</Text></View>
                                <View style={{ flex: 1, ...styles.tableCol }}><Text>{sale?.totalSoldSeat}</Text></View>
                                <View style={{ flex: 1, ...styles.tableCol }}><Text>{sale?.totalFare}</Text></View>
                                <View style={{ flex: 1, ...styles.tableCol }}><Text>{sale?.commissionFee}</Text></View>
                            </View>
                        ))}
                        ...
                        <View style={[styles.tableRow, { borderTop: "1pt solid black" }]} wrap={false}>
                            ...
                            <View style={{ flex: 1, ...styles.tableCol, fontWeight: "bold" }}><Text>{result?.reduce((sum: number, s: any) => sum + (s.totalSoldSeat || 0), 0)}</Text></View>
                            <View style={{ flex: 1, ...styles.tableCol, fontWeight: "bold" }}><Text>{result?.reduce((sum: number, s: any) => sum + (s.totalFare || 0), 0)}</Text></View>
                            <View style={{ flex: 1, ...styles.tableCol, fontWeight: "bold" }}><Text>{result?.reduce((sum: number, s: any) => sum + (s.commissionFee || 0), 0)}</Text></View>
                        </View>
                    </View>
                </View>
...
export default PdfCounterSalesReport;